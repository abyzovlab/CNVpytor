...
_logger = logging.getLogger("cnvpytor.app")
...
class Root:
    ...
    def read_vcf(self, vcf_file, chroms, sample=''):
        ...
        for c in chrs:
            ...
            if not pos is None and len(pos) > 0:
                self.io.save_vcf(pos, ref, alt, nref, nalt, gt, flag, qual)
                count += 1
        return count
        ...
        for c in self.io.rd_chromosomes():
            if c in rd_gc_chromosomes:
                if len(chroms) == 0 or (c in chroms):
                    ...
                    stat = self.io.get_signal(None, None, "RD stat", flag)
                    rd_p, rd_u = self.io.read_rd(c)
                    ...
                    _logger.debug("Decompressing GC correction.")
                    gcat = self.io_gc.get_signal(rd_gc_chromosomes[c], None, "GC/AT")
                    gcp = gcp_decompress(gcat)
                    _logger.debug("Calculating GC corrected RD")
                    gc_corr = self.io.get_signal(None, None, "GC corr", flag)
                    rd_p_corr = rd_p / np.array(list(map(lambda x: gc_corr[int(x)], gcp)))
                    his_p_corr = np.concatenate(
                        (rd_p_corr, np.zeros(bin_ratio - len(rd_p_corr) + (len(rd_p_corr) // bin_ratio * bin_ratio))))
                    his_p_corr.resize((len(his_p_corr) // bin_ratio, bin_ratio))
                    his_p_corr = his_p_corr.sum(axis=1)
    ...
    def his_mask(self, bin_size, chroms=[]):
        his_stat = []
        auto, sex, mt = False, False, False
        rd_gc_chromosomes = {}
        for c in self.io_gc.gc_chromosomes():
                ...
                if len(chroms) == 0 or (c in chroms):
                    ...
                    for (b, e) in mask:
                        for p in range((b - 1) // 100 + 2, (e - 1) // 100):
                            ...
                            his_p_corr[p // bin_ratio] += rd_p[p] / gc_corr[int(gcp[p])]
                    ...
                    his_p_corr = his_p_corr / his_count * bin_ratio
                    # his_p_corr[np.isnan(his_p_corr)] = 0.0
